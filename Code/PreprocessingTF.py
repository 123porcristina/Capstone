...
#Drop Columns
crash=crash[['CRASH_RECORD_ID','RD_NO','CRASH_DATE', 'PRIM_CONTRIBUTORY_CAUSE','POSTED_SPEED_LIMIT', 'TRAFFIC_CONTROL_DEVICE','WEATHER_CONDITION',
...
#Reduced Categories from 40 to 25
print('length:', len(crash.PRIM_CONTRIBUTORY_CAUSE.unique()))
print(crash.PRIM_CONTRIBUTORY_CAUSE.value_counts(normalize=True, dropna=False))
def contributory_cause_new(row):
    val = row['PRIM_CONTRIBUTORY_CAUSE']
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'DISREGARDING STOP SIGN':
        return 'DISREGARDING TRAFFIC SIGNALS'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'DISREGARDING OTHER TRAFFIC SIGNS':
        return 'DISREGARDING TRAFFIC SIGNALS'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'DISREGARDING ROAD MARKINGS':
        return 'DISREGARDING TRAFFIC SIGNALS'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'DISREGARDING YIELD SIGN':
        return 'DISREGARDING TRAFFIC SIGNALS'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'DISREGARDING ROAD MARKINGS':
        return 'DISREGARDING TRAFFIC SIGNALS'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'DISTRACTION - FROM INSIDE VEHICLE':
        return 'DISTRACTION'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'DISTRACTION - FROM OUTSIDE VEHICLE':
        return 'DISTRACTION'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'DISTRACTION - OTHER ELECTRONIC DEVICE (NAVIGATION DEVICE, DVD PLAYER, ETC.)':
        return 'DISTRACTION'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'TEXTING':
        return 'DISTRACTION'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'HAD BEEN DRINKING (USE WHEN ARREST IS NOT MADE)':
        return 'UNDER THE INFLUENCE OF ALCOHOL/DRUGS (USE WHEN ARREST IS EFFECTED)'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'EXCEEDING SAFE SPEED FOR CONDITIONS':
        return 'EXCEEDING AUTHORIZED SPEED LIMIT'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'MOTORCYCLE ADVANCING LEGALLY ON RED LIGHT':
        return 'OTHER'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'OBSTRUCTED CROSSWALKS':
        return 'OTHER'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'PASSING STOPPED SCHOOL BUS':
        return 'OTHER'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'BICYCLE ADVANCING LEGALLY ON RED LIGHT':
        return 'OTHER'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'RELATED TO BUS STOP':
        return 'OTHER'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'CELL PHONE USE OTHER THAN TEXTING':
        return 'DISTRACTION'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'ROAD ENGINEERING/SURFACE/MARKING DEFECTS':
        return 'ROAD'
    if row['PRIM_CONTRIBUTORY_CAUSE'] == 'ROAD CONSTRUCTION/MAINTENANCE':
        return 'ROAD'

    else:
        return val
crash['Contributory_Cause_New'] = crash.apply(contributory_cause_new, axis=1)
print('length new:', len(crash.Contributory_Cause_New.unique()))
print(crash.Contributory_Cause_New.value_counts(normalize=True, dropna=False))
...
person = person[['CRASH_RECORD_ID', 'PERSON_ID','PERSON_TYPE','SEX','AGE',
                 'BAC_RESULT VALUE']]


print(person['AGE'].value_counts(normalize=True, dropna=False))
def new_age(row):
    val = row['AGE']
    if row['AGE'] < 0:
        return np.nan
        ...
        return val
person['Age_New'] = person.apply(new_age, axis=1)
person['Age_New'] = person['Age_New'].fillna(person.AGE.median())
print(person['Age_New'].value_counts(normalize=True, dropna=False))
...
person = person[['CRASH_RECORD_ID', 'PERSON_ID', 'Sex_New','Age_New',
                 'BAC_RESULT VALUE']]
...
print(person.info())
person = person[['CRASH_RECORD_ID', 'Person2','Sex_New','Age_New', 'BAC_RESULT VALUE']]
...
person2 = person2.drop(columns=[('Age_New', 'three'), ('Age_New', 'four'), ('Age_New', 'five'), ('Age_New', 'six'),
                                ('Age_New', 'seven'), ('Age_New', 'eight'), ('Age_New', 'nine'), ('Age_New', 'ten')])
...
def new_age(row):
    vala = row[('Age_New', 'one')]
    valb =  row[('Age_New', 'two')]
    val = np.mean([vala,valb])
    return val
person2['AGE2'] = person2.apply(new_age, axis=1)

print(person2.AGE2.value_counts(dropna=False))
...
merge2=merge.drop(columns=['POSTED_SPEED_LIMIT','TRAFFIC_CONTROL_DEVICE', 'WEATHER_CONDITION', 'LIGHTING_CONDITION',
                           ...
                           'Sex_New','Sex_New.1','BAC_RESULT VALUE', 'BAC_RESULT VALUE.1', 'Age_New', 'Age_New.1'])
